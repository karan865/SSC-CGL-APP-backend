...
export const startMockTestHandler = async (
  ...
  try {
    ...
    const result = await startMockTest(testType || 'TIER_1');

    res.status(200).json(
      successResponse('SSC CGL Tier-1 Mock Test started successfully', result)
    );
  } catch (error: any) {
...
export const getMarkedQuestionsHandler = async (
  ...
  try {
    ...
    const result = await getMarkedQuestions(sessionId);
    res.status(200).json(
