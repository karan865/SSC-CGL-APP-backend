...
export async function recordMockTestAttempts(
  ...
  for (const item of answeredItems) {
    ...
    const marks = isCorrect ? 2 : -0.5; // Official SSC CGL Tier-1 scoring
 ...
 */
export async function getUserPerformance(userId: string = 'guest_default'): Promise<UserPerformanceSummary> {
  const normalizedUserId = userId?.trim() || 'guest_default';

  // 1. Fetch all subjects and topics for complete mapping
  const allSubjects = await Subject.find({ isActive: true }).sort({ order: 1 });
  const allTopics = await Topic.find({ isActive: true }).sort({ order: 1 });
  ...
  // Merge topic aggregates into full structure
...
export async function getRecommendedPractice(
  ...
  const normalizedUserId = userId?.trim() || 'guest_default';
  const performance = await getUserPerformance(normalizedUserId);
  ...
  if (performance.weakTopics.length > 0) {
    ...
    // New student with 0 attempts: pick Quantitative Aptitude -> Percentage
    const quantSub = await Subject.findOne({ slug: 'quantitative-aptitude', isActive: true });
    const percentTopic = await Topic.findOne({
      subjectId: quantSub?._id,
      slug: 'percentage',
      isActive: true,
    });

    if (percentTopic && quantSub) {
      targetTopic = {
        topicId: percentTopic._id.toString(),
        topicName: percentTopic.name,
        topicSlug: percentTopic.slug,
        subjectId: quantSub._id.toString(),
        subjectName: quantSub.name,
        subjectSlug: quantSub.slug,
        attempted: 0,
        correct: 0,
        wrong: 0,
        accuracy: 0,
      ...
      recommendedDifficulty = 'Easy';
      reason = 'Start your SSC CGL preparation with high-yield Percentage practice!';
    }
  }

  // Fallback if no topic found
  if (!targetTopic) {
    const fallbackTopic = await Topic.findOne({ isActive: true });
