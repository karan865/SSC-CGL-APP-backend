...
export async function selectPracticeQuestions(
  ...
  const isMiniMock = subjectId === 'mini-mock' || topicId === 'mixed';
  
  let resolvedExamId = examId;
  if (isMiniMock && !resolvedExamId) {
    // Infer examId from the user's active daily study plan
  ...
  if (!isMiniMock && (!mongoose.Types.ObjectId.isValid(subjectId) || !mongoose.Types.ObjectId.isValid(topicId))) {
    throw new Error('INVALID_ID');
  }

  let subObjId: mongoose.Types.ObjectId | undefined;
  let topicObjId: mongoose.Types.ObjectId | undefined;
  
  if (!isMiniMock) {
    subObjId = new mongoose.Types.ObjectId(subjectId);
  ...
  if (!isMiniMock) {
    matchCriteria.subjectId = subObjId;
    matchCriteria.topicId = topicObjId;
    topicCriteria.subjectId = subObjId;
    topicCriteria.topicId = topicObjId;
  }

  if (resolvedExamId && mongoose.Types.ObjectId.isValid(resolvedExamId)) {
    matchCriteria.examId = new mongoose.Types.ObjectId(resolvedExamId);
  ...
  // 2. Fetch distinct question IDs attempted by this user in this topic/exam
  const attemptFilter: any = { userId: normalizedUserId };
  if (!isMiniMock) {
    attemptFilter.topicId = topicObjId;
  } else if (matchCriteria.examId) {
    attemptFilter.examId = matchCriteria.examId;
  }
  const attemptedQuestionIds: mongoose.Types.ObjectId[] = await QuestionAttempt.distinct('questionId', attemptFilter);
  ...
  if (selectedQuestions.length < targetCount) {
    ...
    if (!isMiniMock) {
      otherUnseenFilter.subjectId = subObjId;
      otherUnseenFilter.topicId = topicObjId;
    }
    ...
    if (selectedQuestions.length < targetCount) {
      ...
      if (!isMiniMock) {
        otherReviewFilter.subjectId = subObjId;
        otherReviewFilter.topicId = topicObjId;
      }
