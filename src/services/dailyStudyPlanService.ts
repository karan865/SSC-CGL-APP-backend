...
export async function generateDailyPlan(
  ...
  if (!forceRegenerate) {
    const existingPlan = await DailyStudyPlan.findOne({ userId: normalizedUserId, dateKey });
    if (existingPlan) {
      return existingPlan;
    }
  }
  ...
  if (attempts < 5) {
    ...
    try {
      const dueData = await getDueRevisions(normalizedUserId, 15);
  ...
  const plan = await DailyStudyPlan.findOneAndUpdate(
    { userId: normalizedUserId, dateKey },
    {
 ...
 */
export async function getTodayPlan(userId: string): Promise<IDailyStudyPlan> {
  const normalizedUserId = userId?.trim() || 'guest_default';
  const dateKey = getTodayDateKey();

  const existing = await DailyStudyPlan.findOne({ userId: normalizedUserId, dateKey });
  if (existing) {
    return existing;
  }

  return generateDailyPlan(normalizedUserId, 35, false);
}

/**
 * Formats daily study plan progress for API responses.
 */
export async function getPlanProgress(userId: string): Promise<PlanProgressSummary> {
